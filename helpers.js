...
const getData = filename => new Promise((resolve) => {
...
const parseData = data => {
    ...
    .map(worker => [worker[0], worker[1].replace(' ', '').replace('[', '').replace(']', '').split(',')])
    .map(worker => [worker[0], worker[1].map(times => times.replace(' ', '').trim().split('/'))])

    // insert parsed data to the array
    parsedData.map((worker, index) => workers[index] = {id: worker[0], times: worker[1]})
...
const earliestIntervalStart = (data) => {
    ...
    // items are sorted
    let times = []
    data.map(item => item.times.map(interval => times = [...times, interval[0]]))
...
const latestIntervalEnd = (data) => {
    ...
    // items are sorted
    let times = []
    data.map(item => item.times.map(interval =>  times = [...times, interval[1]]))
    times = times.map(time => Date.parse(time))
    times.sort()
...
const overlapingIntervals = (data) => {
    ...
    data.map(item => item.times.map(interval => 
        times = [...times, {id: item.id, interval: [new Date(Date.parse(interval[0])), new Date(Date.parse(interval[1]))]}]))
...
// returns unique interescting ranges
 ...
 module.exports = {getData, parseData, overlapingIntervals, earliestIntervalStart, latestIntervalEnd}